...
def update_payment(payment_id):
    try:
        data = request.json
        mpesa_number = data.get("mpesa_number")
        new_payment = float(data.get("amount_paid", 0))  # the newly paid amount
...
def update_balance(payment_id):
    try:
        data = request.get_json()
        extra_paid = float(data.get("amount_paid", 0))  # New installment
        mpesa_number = data.get("mpesa_number")  # Optional, if available
        ...
        if mpesa_number:
            update_data["mpesa_number"] = mpesa_number
            ...
            if not fully_paid:
                ...
                <html>
                  <body>
                    ...
                    <a href="https://trustpay-backend.onrender.com/update-balance/{payment_id}"
                       style="padding:10px 20px; background-color:orange; color:white; text-decoration:none; border-radius:5px;">
                       💳 Pay Balance
                    </a>
                    <br><br>
